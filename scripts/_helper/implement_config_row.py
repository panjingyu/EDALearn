...
def parse_args():
    ...
    # other options
    arg_parser.add_argument('--random-idx', '-r', action='store_true',
                            help='Randomly choose a version index.')
    arg_parser.add_argument('--force-override', '-f', action='store_true',
                            help='Force override existing results.')
    arg_parser.add_argument('--skip-dbg', '-k', action='store_true',
                            help='Skip designs found in ./dbg_designs')
...
def main(args):
    ...
    if args.random_idx:
        global_ver_idx = random.randint(0, len(global_config.index) - 1) if args.global_ver_idx is None else args.global_ver_idx
        place_ver_idx = random.randint(0, len(place_config.index) - 1) if args.place_ver_idx is None else args.place_ver_idx
        cts_ver_idx = random.randint(0, len(cts_config.index) - 1) if args.cts_ver_idx is None else args.cts_ver_idx
    else:
    ...
    for idx, d in enumerate(designs):
        # if not (d.startswith('l') or d.startswith('D')):
        # if not d starts with lower letter (except b or s) or D, skip
        # if not (d[0].islower() and d[0] not in ['b', 's'] or d[0] == 'D'):
        ...
        if os.path.isdir('/'.join(['./designs', d])):
            ...
            log_file.flush()
            if d in skipped_designs and args.skip_dbg:
